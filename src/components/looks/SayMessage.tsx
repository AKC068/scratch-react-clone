...
interface SayMessageProp {
  character: any;
  comp_id: string;
}

const SayMessage = (sayMessageProp: SayMessageProp) => {
  ...
  const displayMessage = () => {
    const el = document.getElementById(
      `${sayMessageProp.character.active}-message-box`
    );
    const el2 = document.getElementById(
      `${sayMessageProp.character.active}-message-box1`
    );

    if (
      state.show_msg &&
      state.character_id === sayMessageProp.character.active
    ) {
    ...
    window.clearTimeout(sayMessageProp.comp_id);
