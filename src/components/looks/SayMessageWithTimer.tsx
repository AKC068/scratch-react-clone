...
interface SayMessageTimerProp {
  character: any;
  comp_id: string;
}

const SayMessageWithTimer = (sayMessageTimerProp: SayMessageTimerProp) => {
  ...
  const displayMessage = () => {
    const el = document.getElementById(
      `${sayMessageTimerProp.character.active}-message-box`
    );
    const el2 = document.getElementById(
      `${sayMessageTimerProp.character.active}-message-box1`
    );
  ...
  return (
    <Paper elevation={3} className="w-56">
      <div className="rounded text-center bg-purple-500 p-2 my-3">
        ...
        <div
          id={sayMessageTimerProp.comp_id}
