...
interface ThinkWithTimerProp {
  character: any;
  comp_id: string;
}

const ThinkWithTimer = (thinkWithTimerProp: ThinkWithTimerProp) => {
  ...
  const displayMessage = () => {
    const el = document.getElementById(
      `${thinkWithTimerProp.character.active}-message-box`
    );
    const el2 = document.getElementById(
      `${thinkWithTimerProp.character.active}-message-box1`
    );
    if (
      state.show_msg &&
      state.character_id === thinkWithTimerProp.character.active
    ) {
